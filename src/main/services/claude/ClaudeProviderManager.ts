...
/**
 * 设置 Claude model
 * @param model - 'opus' | 'sonnet' | 'haiku' 或 null 清除
 */
export function setClaudeModel(model: string | null): boolean {
  try {
    const settingsPath = getClaudeSettingsPath();
    let settings: ClaudeSettings = {};
    ...
    console.log(`[ClaudeProviderManager] Set model: ${model}`);
    return true;
  } catch (error) {
    console.error('[ClaudeProviderManager] Failed to set model:', error);
    return false;
  }
}

/**
 * 获取当前 Claude model 设置
 */
